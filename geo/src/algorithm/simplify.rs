...
mod hegel_props {
    ...
    fn epsilons() -> impl PrintableGenerator<f64> {
        hegel::one_of!(
    ...
    // triangle inequality — the property `geo/fuzz/fuzz_targets/simplify.rs`
    // checks for polygon rings.
    //
    // Coordinates are capped at 1e150 to keep the cross product inside
    // `Euclidean.distance(coord, &Line)` finite. Above that `simplify` trips
    // its own `debug_assert_ne!(farthest_index, 0)`, the debug-build half of
    // #1606, on `LINESTRING(0 0,9335 9335,0 1.9257559023699152e304)` for one.
    #[hegel::test]
    ...
    // KNOWN FAILURE, #1606 (open): `simplify` drops the middle vertex, which
