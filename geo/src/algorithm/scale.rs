...
mod hegel_props {
    use super::Scale;
    use crate::utils::hegel_gens::{points, star_polygons};
    use crate::{AffineOps, AffineTransform, Area, BoundingRect};
    use hegel::generators::{self, PrintableGenerator};

    fn factors() -> impl PrintableGenerator<f64> {
    ...
    fn scale_xy_fixes_the_bounding_box_centre(tc: hegel::TestCase) {
        ...
        let centre = polygon
            ...
            .center();
        let x = tc.draw(factors());
        let y = tc.draw(factors());
        let scaled = polygon.scale_xy(x, y);
