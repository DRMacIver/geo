...
mod hegel_props {
    ...
    use crate::utils::hegel_gens::{grid_coords, star_rings};
    ...
    fn make_winding_order_either_keeps_or_reverses_the_coords(tc: hegel::TestCase) {
        let ring = tc.draw(star_rings());
        let clockwise = tc.draw(hegel::generators::booleans());
    ...
    // KNOWN FAILURE, #1608 (open): `triangle_winding_order` reports a
