...
mod hegel_props {
    ...
    use crate::utils::hegel_gens::{monotone_line_strings, star_rings};
    use crate::{Euclidean, Length};
    use hegel::generators::{self, PrintableGenerator};

    fn iterations() -> impl PrintableGenerator<usize> {
    ...
    fn smoothing_preserves_the_ends_of_an_open_line_string(tc: hegel::TestCase) {
        ...
        let iterations = tc.draw(iterations());
        let smoothed = line_string.chaikin_smoothing(iterations);
    ...
    fn smoothing_a_ring_never_lengthens_it(tc: hegel::TestCase) {
        let ring = tc.draw(star_rings());
        let iterations = tc.draw(iterations());
        let smoothed = ring.chaikin_smoothing(iterations);
    ...
    fn smoothing_keeps_a_closed_ring_closed(tc: hegel::TestCase) {
        let ring = tc.draw(star_rings());
        let iterations = tc.draw(iterations());
        assert!(ring.chaikin_smoothing(iterations).is_closed());
    }
