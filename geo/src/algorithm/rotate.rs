...
mod hegel_props {
    use super::Rotate;
    use crate::utils::hegel_gens::{monotone_line_strings, points, star_polygons};
    use crate::{AffineOps, AffineTransform, Area, Euclidean, Length};
    use hegel::generators::{self, PrintableGenerator};

    fn degrees() -> impl PrintableGenerator<f64> {
        generators::floats::<f64>()
            .min_value(-720.0)
            .max_value(720.0)
    }

    // "a rotated point should always equal itself" — the comment in
    // `test_rotate_points` above: for a `Point` both pivots are
    ...
    fn rotating_a_point_about_itself_returns_the_point(tc: hegel::TestCase) {
        let point = tc.draw(points(1e3));
        let angle = tc.draw(degrees());
        for rotated in [
