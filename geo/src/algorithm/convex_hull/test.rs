...
mod hegel_props {
    ...
    use crate::utils::hegel_gens::{canonical_ring, coords, grid_point_sets};
    ...
    // The orientation properties use `grid_point_sets`: on the integer grid
    // every product and difference `quick_hull` forms is exact, so the
    // magnitude-dependent failures pinned below (#1566) cannot arise.
    #[hegel::test]
    ...
    // The hull of a convex set is that set, so hulling twice adds nothing. The
    // trait fixes the orientation of the hull but not which vertex it starts
    // from, hence `canonical_ring`.
    #[hegel::test]
    ...
    // KNOWN FAILURE, #1566 (open) reports the wrong hull above, not this
    // panic. Both come from the pivot arithmetic: here the score overflows to
    // infinity for finite coordinates, `inf - inf` gives NaN, and the
    // `max_by(|a, b| a.partial_cmp(b).unwrap())` in `hull_set` unwraps
    // `None`, in release builds too. A fix for the reported hull that leaves
    // the `unwrap` in place leaves this test red.
    #[test]
    #[ignore = "#1566: convex_hull panics at large finite coordinates (the issue reports the wrong hull, not the panic)"]
