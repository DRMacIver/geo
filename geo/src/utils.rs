...
/// Generators shared by the [hegel](https://hegel.dev) property tests
/// throughout the crate (the dev-dependency is the `hegeltest` package).
///
/// A `#[hegel::test]` is a `#[test]` whose body runs repeatedly over inputs
/// drawn with `tc.draw(..)`. When a case panics, hegel shrinks it, prints each draw bound
/// by a `let` in the test body as `let name = value;` above the panic message,
/// and saves the case under `.hegel/` (gitignored) to replay on the next run.
/// Geometries print in geo-types' `Debug` form, which is WKT
/// (`POLYGON((..))`) for the standard types and `RECT(..)`, `LINE(..)` or
/// `COORD(..)` for geo's own, so a counterexample reads directly but is
/// transcribed by hand into a plain `#[test]`. `tc.assume(cond)` discards
/// the current case when `cond` is false and draws another.
///
/// A hegel `Generator` draws a value; a `PrintableGenerator` also knows how
/// to print it in a counterexample, which `tc.draw` requires, and
/// `print_as_debug()` turns the former into the latter using `Debug`.
/// Geometry types are foreign to hegel, so every generator here ends in
/// `print_as_debug()` and draws its parts with `draw_silent`, so that a
/// counterexample shows the assembled geometry rather than the floats it was
/// built from.
#[cfg(test)]
pub(crate) mod hegel_gens {
    ...
    /// Simple closed rings, counter-clockwise by construction.
    ///
    ...
    pub(crate) fn star_rings() -> impl PrintableGenerator<LineString<f64>> {
        hegel::compose!(|tc| {
            let centre = draw_coord(tc, 1e3);
            let mut ring = draw_star_ring(tc, centre, 0.5, 1e3);
            ring.close();
            ring
        })
        .print_as_debug()
    ...
    /// Simple polygons without holes: a star ring as the exterior.
    pub(crate) fn star_polygons() -> impl PrintableGenerator<Polygon<f64>> {
        hegel::compose!(|tc| { Polygon::new(tc.draw_silent(star_rings()), vec![]) })
            .print_as_debug()
    }
    ...
    /// Points with the coordinates of `coords(max_coord)`.
    pub(crate) fn points(max_coord: f64) -> impl PrintableGenerator<Point<f64>> {
        coords(max_coord).map(Point::from).print_as_debug()
    }

    /// Points with the coordinates of `grid_coords()`.
    pub(crate) fn grid_points() -> impl PrintableGenerator<Point<f64>> {
        grid_coords().map(Point::from).print_as_debug()
    }

    /// Up to 64 grid coordinates, so repeated and exactly collinear points are
    /// common.
    pub(crate) fn grid_point_sets() -> impl PrintableGenerator<Vec<Coord<f64>>> {
        generators::vecs(grid_coords()).max_size(64)
    }

    /// A closed ring rotated to start at its lexicographically least vertex,
    /// with the repeated closing coordinate dropped, so two rings of the same
    /// polygon compare equal whichever vertex each starts from.
    pub(crate) fn canonical_ring(ring: &LineString<f64>) -> Vec<Coord<f64>> {
        if ring.0.is_empty() {
            return Vec::new();
        }
        let open = &ring.0[..ring.0.len() - 1];
        let start = super::least_index(open);
        open[start..]
            .iter()
            .chain(&open[..start])
            .copied()
